...
float pt1_element(int axis, float delta, uint8_t f_cut) {
    ...
    RC = 1.0f / ( 2.0f * (float)M_PI * (float) f_cut );
        ...
        uint16_t max_angle_inclination, rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    for (axis = 0; axis < 3; axis++) {
        ...
        if (pidProfile->main_cut_hz) {
            deltaSum = pt1_element(axis, deltaSum, pidProfile->main_cut_hz);
        }
        ...
        uint16_t max_angle_inclination, rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    for (axis = 0; axis < 3; axis++) {
        ...
        if (pidProfile->main_cut_hz) {
            deltaSum = pt1_element(axis, (float) deltaSum, pidProfile->main_cut_hz);
        }
            ...
            rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    for (axis = 0; axis < 2; axis++) {
        ...
        if (pidProfile->main_cut_hz) {
        	DTerm = pt1_element(axis, (float) DTerm, pidProfile->main_cut_hz);
        }
